// STRING

// const furstValue = String(5);

// console.log(typeof furstValue);
// const obj = {
//     foo: 67,
// };

// console.log(typeof String(obj.foo));

// NUMBER

// let testValue;

// console.log(typeof testValue);

// testValue = Number(testValue);

// console.log(testValue);

// const isUser = true;
// const isAdmin = false;

// console.log(Number(isUser),Number(isAdmin));

// const voidString = "";

// console.log(Number(voidString));

// BOOLEAN

// const value = "";

// console.log(typeof Boolean(value),Boolean(value));

// const value =30;

// console.log(typeof Boolean(value),Boolean(value));

// const obj = [1].length;
// console.log(Boolean(obj));

// console.log(2**2);

//  Унарный плюс

// const value = 5;

// console.log(typeof value, typeof +value, typeof Number(value));

// Бинарный плюс

// const sum = 5+6;
// console.log(sum);

// sum = 1 + true;
// console.log(sum);

// sum = 5 + null;
// console.log(sum);

// const stringValue ="12x";
// const numberValue = 34;
// console.log(+stringValue + numberValue);
// console.log(typeof(numberValue + +stringValue));

// const firstName = "Alex";
// const lastName = "Shirokov";

// console.log(firstName+" "+lastName);

// console.log(`${firstName} ${lastName}`);/* лучше использовать так */

// Операции сравнения

// console.log(5 !== 8);  /* != -  не равно */

// console.log(false == 0);

// const firstString = "a"; /* английские */
// const secondString = "а";/* русские */

// console.log(firstString < secondString);

// const a = 0;
// const b = "0";

// console.log(a == b);
// console.log(a === b);

// Практика


